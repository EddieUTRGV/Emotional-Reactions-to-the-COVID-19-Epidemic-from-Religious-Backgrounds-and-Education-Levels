...
```{r}
table(data.xls$ONLY_R)

chisq.test(table(data.xls$ONLY_R))$expected

chisq.test(table(data.xls$ONLY_R))

ggplot(data.xls, aes(x = ONLY_R)) +
  geom_bar() +
...
table(data.xls$CVD_EXAG)

chisq.test(table(data.xls$CVD_EXAG))$expected

chisq.test(table(data.xls$CVD_EXAG))

ggplot(data.xls, aes(x = CVD_EXAG)) +
  geom_bar() +
  labs(title = "Observed Frequencies", x = "Category", y = "Count")
```
